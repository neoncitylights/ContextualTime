	...
	public class Parser {
		...
		public TimePhrase Parse( string input ) {
			...
			return words[0].ToLowerInvariant() switch {
				...
				TokenOn => new OnPhrase( 
					DateTime.Now.DayOfWeek, 
					Enum.Parse<DayOfWeek>( words[1],true ) ),
		...
		private NextPhrase GetNewPhrase( string[] words ) {
			return words.Length == 3
				? new NextPhrase(
					Int32.Parse( words[1] ),
					Enum.Parse<UnitOfTime>(
						NormalizeEnPluralUnit( words[2] ), true ) )
				: new NextPhrase( Enum.Parse<UnitOfTime>( 
					NormalizeEnPluralUnit( words[1] ), true ) );
		}
		...
		private LastPhrase GetLastPhrase( string[] words ) {
			return words.Length == 3
				? new LastPhrase(
					...
					Enum.Parse<UnitOfTime>( 
						NormalizeEnPluralUnit( words[2] ), true ) )
				: new LastPhrase( Enum.Parse<UnitOfTime>( 
					NormalizeEnPluralUnit( words[1] ), true ) );
		}
	}
}
